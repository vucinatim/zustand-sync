...
function App() {
  ...
  const { connectionStatus, clientId } = useGameStore(); // Select multiple items
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      // --- MODIFIED LOGIC ---
      // Use the dynamic clientId from our local state to find our character
      const myCharacter = characters.find((c) => c.id === clientId);

      if (!myCharacter) return; // Don't do anything if our character isn't there yet
      // --- END MODIFIED LOGIC ---
      ...
      // Ensure character stays within bounds (accounting for character size)
      ...
      // --- MODIFIED LOGIC ---
      // Pass our dynamic clientId to the moveCharacter action
  ...
  }, [characters, gameActions, clientId]); // Add clientId to the dependency array
    ...
    <div className="w-screen h-screen bg-gray-900 text-white font-sans overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
        <div className="absolute top-6 left-6 bg-black/80 backdrop-blur-sm p-4 rounded-xl shadow-2xl border border-gray-700 pointer-events-auto">
          ...
          <p className="text-xs text-gray-400 mb-3">Client ID: {clientId}</p>
          <p className="text-sm text-gray-300 mb-4">
            Use <span className="font-mono bg-gray-700 px-1 rounded">↑↓←→</span>{" "}
        ...
        {/* --- UI controlled by the local store --- */}
        {isInstructionsOpen && (
